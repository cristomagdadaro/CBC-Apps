...
## Tracker Updates (2026-04-13)
- [ID-2026-04-13-001] Resolved. Guest inventory remaining-stocks responses could include negative `remaining_quantity` rows when outgoing transactions existed without matching incoming stock. The guest-facing controller path now defaults to `min_remaining=0` unless the caller explicitly requests a threshold, and feature coverage was added to prevent regressions.
- [ID-2026-04-13-002] Resolved (Ops/Infra). Public-host update requests (`PUT`) were blocked by ModSecurity CRS method enforcement (`id 911100`, allowed methods limited to `GET HEAD POST OPTIONS`) and surfaced as `949110` anomaly-score 403 responses. The public server policy was updated to allow REST update verbs for the API surface, and update endpoints now succeed on `onecbc.philrice.gov.ph`.

## Tracker Updates (2026-04-14)
- [ID-2026-04-14-001] Resolved (Ops/Infra). The public deployment was still rejecting REST update verbs on `/api/*` routes because ModSecurity CRS method enforcement (`REQUEST-911-METHOD-ENFORCEMENT`, id `911100`) treated `PUT` as disallowed and escalated the request to `949110` anomaly blocking. The allow-list policy for the API surface now includes `PUT`, `PATCH`, and `DELETE`, and the previously failing inventory, users, personnel, and options update requests now complete successfully on `onecbc.philrice.gov.ph`.
- [ID-2026-04-14-002] Resolved. `resources/js/Pages/Forms/components/GuestCard.vue` was producing Vue runtime warnings during the form-update flow because the unavailable-state renderer dereferenced imported icon constructors directly in the template. The renderer now resolves the icon through `getStepIcon()`, so the warning path is closed.
- [ID-2026-04-14-003] Resolved. `resources/js/Components/CameraScanner.vue` was QR-only and struggled with Code 128 inventory labels because the browser decoder exposed the raw payload with placeholder/control characters. The shared scanner now uses the `BarcodeDetector` API/polyfill path, supports both QR and barcode formats, and normalizes Code 128 inventory values like `CBC-01-229333` at the scan boundary.

## Tracker Updates (2026-04-15)
- [ID-2026-04-15-001] Resolved. `app/Repositories/TransactionRepo.php::getRemainingStocks()` was fragmenting a single barcode-scoped stock identity into multiple rows because it grouped by `transactions.unit` and `transactions.barcode_prri` in addition to the barcode itself. Remaining-stock and recount-summary queries now aggregate by the barcode identity, prefer canonical incoming metadata (`unit`, `barcode_prri`, `project_code`), and exclude blank barcodes from the barcode-scoped stock summary so duplicate rows with mismatched quantities no longer appear for the same stock code.
- [ID-2026-04-15-002] Resolved. Recounting transactions often have no `personnel_id`, so transaction views that only rendered personnel names showed blank actors even though `user_id` identified who performed the adjustment. Transactions now expose a shared `actor_display_name` resolver that prefers personnel names and falls back to the linked user, and the inventory datatable, outgoing recent-transactions panel, and dashboard recent-transactions panel now render that shared actor label.
- [ID-2026-04-15-003] Resolved. The unified notification dispatcher was fan-out only, so staff-facing events such as supply checkout, equipment-log lifecycle updates, form responses, and certificate summaries produced one queued email and one inbox message per resolved recipient. Notification domains can now opt into grouped delivery mode, grouped sends use a single queued message with BCC for recipient privacy, and per-recipient `notification_logs` rows are still created and updated so audit visibility remains intact.
- [ID-2026-04-15-004] Resolved. Equipment logger lifecycle emails were using custody wording opposite the current product UI and service semantics: creating an active usage log corresponds to “Check In Equipment” while completing the log corresponds to “Check Out Equipment.” `SendEquipmentLogLifecycleNotification` now maps `created` to `checked in` and `completed` to `checked out`, and regression coverage was added so the email labels stay aligned with the logger screens.
- [ID-2026-04-15-005] Resolved. Grouped notification sends could exceed the previous 90-second worker timeout, and the database queue lease matched that same threshold. When a grouped mail job timed out, the same job UUID could be failed more than once and hit the `failed_jobs.uuid` unique constraint. `DeliverNotificationMessageJob` now declares a 240-second timeout and fails deterministically on timeout, `config/queue.php` now exposes `QUEUE_RETRY_AFTER` with a safer 300-second default, and the production README/example env now document the required `retry_after > worker timeout` relationship for notification and certificate workers.
...
## Tracker Updates (2026-05-05)
- [ID-2026-05-05-001] Resolved. The guest supplies checkout page and the internal outgoing inventory page were treating the Storage Room dropdown as a synthetic barcode search instead of using the backend's dedicated `storage_location_id` filter, so selecting a room often failed to narrow results to that room's registered stock. Both pages now send `storage_location_id` directly through the existing search payload, preserve the rest of the current filtering structure, and the guest remaining-stocks route is covered by a regression test for room-specific filtering.
- [ID-2026-05-05-002] Resolved. The public landing-page service cards had no lightweight operational counters, so guests could not immediately see active Lab/ICT equipment usage or the current day's supplies checkout volume. `Welcome.vue` service cards now support optional badge bubbles, and the welcome route injects live counts for laboratory active equipment, ICT active equipment, and today's outgoing supply transactions.
- [ID-2026-04-20-003] Resolved. Equipment Logger availability had been attached to `items`, which incorrectly treated a shared item master record as the source of truth even when separate incoming transactions/project codes needed different sharing rules. The logger mode field now lives on `transactions`, incoming transaction forms read the allowed values from the Options module (`equipment_logger_modes`), item forms no longer hardcode logger choices, and the laboratory dashboard consumes the same option-backed labels. Remaining architectural note: the runtime logger identity is still item-centric (`laboratory_equipment_logs.equipment_id -> items.id`), so a future refactor should consider transaction/barcode-scoped logger identities if per-procurement concurrency becomes a product requirement.

## Tracker Updates (2026-05-21)
- [ID-2026-05-21-001] Resolved. Personnel registration was an administrator-only workflow, which forced staff to manually enter guest/OJT/personnel records and provided no self-service email ownership check before a person became available in inventory-linked workflows. A public personnel registration route now accepts guest submissions, sends a signed verification email, waits for email verification, and exposes an authenticated inventory approval queue where administrators can approve or reject records. Approval creates the canonical `personnels` record, copies `email_verified_at`, and assigns outsider/OJT CBC IDs through the shared personnel ID service instead of trusting guest-supplied generated IDs.

## Tracker Updates (2026-06-22)
- [ID-2026-06-22-001] Resolved. 23 npm security alerts eliminated: `axios` bumped from `1.15.0` to `1.18.0` (fixes all prototype-pollution, proxy-credential-leak, SSRF, and header-injection alerts); `vitest` bumped from `4.0.18` to `4.1.9` (fixes critical arbitrary-file-read/RCE CVE-2026-47429 and CVE-2026-53633). All transitive alerts (`follow-redirects`, `form-data`, `qs`) are resolved as a result of the `axios` update. `npm audit` now reports 0 vulnerabilities. Note: npm must be invoked through Linux nvm (`source ~/.nvm/nvm.sh && nvm use v24.14.1`) from within WSL to avoid Windows-node path conflicts with the Linux `node_modules` symlinks.
...
- [ID-2026-04-21-003] Resolved. Equipment Logger administrators can now quick-edit `equipment_logger_mode` directly from the dashboard equipment list: double-clicking the mode badge opens a dedicated modal, the dashboard calls a dedicated authenticated API endpoint, and the backend updates the latest incoming transaction for the selected equipment row instead of forcing users through the full inventory transaction edit flow.
- [ID-2026-04-21-004] Resolved. Research study/experiment page links previously used numeric IDs in URLs, which made route identifiers inconsistent with the module’s visible `code` fields and encouraged brittle deep links. `ResearchStudy` and `ResearchExperiment` now expose `route_identifier` (code-first, id fallback), support dual route binding (`code` or `id`), and the research frontend now navigates show/create flows using code-based identifiers while preserving legacy id compatibility.
- [ID-2026-04-21-005] Resolved. Barcode/QR rendering logic was duplicated across Inventory BarcodePrint and Research Sample Inventory (inline `JsBarcode`/`QrcodeVue` blocks plus module-specific markup), increasing drift risk when changing label behavior. A shared `resources/js/Components/QrBarCode.vue` component now centralizes barcode/QR rendering and is consumed by Inventory `LabelCard` and Research scanner snapshot UI; existing print workflows remain intact because module-specific sizing/layout orchestration is unchanged.
...
- `php artisan test tests/Unit/Services/Laboratory/LaboratoryLogServiceTest.php`: 8 passed, 13 assertions, 0 failures (`xdebug.mode=coverage` warning only on this workstation).
- `php artisan test tests/Feature/Inventory/TransactionsTest.php`: 4 passed, 32 assertions, 0 failures (`xdebug.mode=coverage` warning only on this workstation).
- `php artisan test tests/Feature/Inventory/ItemsTest.php -v`: 1 passed, 9 assertions, 0 failures (`xdebug.mode=coverage` warning only on this workstation).
- `php artisan test tests/Feature/Research/ResearchAccessTest.php`: 7 passed, 25 assertions, 0 failures after adding study/experiment code-route binding coverage (`XDEBUG_MODE=coverage` warning only on this workstation).
- `npm run build`: passed after introducing shared `QrBarCode` and code-based study/experiment route identifiers; Vite completed successfully on this workstation (chunk-size warnings only).
...
- Public deployment verification (Ops): ModSecurity audit entries for failing update calls identified root trigger `REQUEST-911-METHOD-ENFORCEMENT` (`id 911100`) before `949110`; after WAF method-policy adjustment, the previously failing API `PUT` updates completed successfully on `onecbc.philrice.gov.ph`.
