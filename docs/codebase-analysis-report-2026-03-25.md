...
- [ID-2026-04-13-002] Deferred (Ops/Infra). Public-host update requests (`PUT`) return `403` across multiple API modules while equivalent flows work on the trusted local host. Route/controller and request authorization code paths do not enforce a verb-specific block in-app; likely cause is upstream public-server method filtering (reverse proxy/WAF). Confirm by checking whether the public 403 payload is HTML/proxy-generated vs Laravel JSON and review web-server `limit_except`/mod_security rules for `PUT`.
