...
<template>
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center py-3 sm:py-4 select-none gap-4 sm:gap-6 w-full">
        <div class="leading-tight flex flex-col min-w-0 max-w-full">
          <div v-if="normalizedBreadcrumbs.length" class="mb-1.5 flex flex-wrap items-center gap-2 text-[0.65rem] sm:text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              <template v-for="(breadcrumb, index) in normalizedBreadcrumbs" :key="`${breadcrumb.label}-${index}`">
                  ...
                  <span v-else class="text-slate-800 dark:text-slate-200">
                      {{ breadcrumb.label }}
                  </span>
              </template>
          </div>
          <Link v-if="resolvedRouteLink" :href="resolvedRouteLink" class="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors truncate">
              {{ title }}
          </Link>
          <h1 v-else class="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight truncate">
              {{ title }}
          </h1>
          <p v-if="subtitle" class="mt-1 text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400 truncate">
              {{ subtitle }}
          </p>
        </div>
