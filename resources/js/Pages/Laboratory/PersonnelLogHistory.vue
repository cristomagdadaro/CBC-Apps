...
export default {
    ...
    computed: {
        ...
        statCards() {
            return [
                { label: 'Total Logs', value: this.personnelSummary.total_logs ?? 0, color: 'text-indigo-600 dark:text-indigo-400' },
                { label: 'Active', value: this.personnelSummary.active_logs ?? 0, color: 'text-emerald-600 dark:text-emerald-400' },
                { label: 'Overdue', value: this.personnelSummary.overdue_logs ?? 0, color: 'text-rose-600 dark:text-rose-400' },
                { label: 'Completed', value: this.personnelSummary.completed_logs ?? 0, color: 'text-slate-600 dark:text-slate-400' },
            ];
        },
    },
};
                ...
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-8">
                    ...
                    <div class="space-y-6 flex-1">
                        <div>
                            ...
                            <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">{{ personnelSummary.full_name }}</h1>
                            <p class="mt-1 text-sm font-medium text-slate-500 dark:text-slate-400">{{ personnelSummary.position || 'No position recorded' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white">{{ personnelSummary.employee_id || '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white truncate">{{ personnelSummary.email || '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white">{{ personnelSummary.phone || '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white truncate">{{ personnelSummary.address || '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white">{{ personnelSummary.first_logged_at || '-' }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-2.5 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                <LuClock class="w-4 h-4 text-slate-400 mt-0.5" />
                                <div>
                                    ...
                                    <p class="text-sm font-medium text-slate-900 dark:text-white">{{ personnelSummary.last_logged_at || '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 w-full lg:w-[26rem] shrink-0">
                        ...
                        >
                            <p class="text-[0.65rem] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-1.5">{{ card.label }}</p>
                        ...
                        <template #cell-equipmentName="{ row }">
                            <div class="py-2 w-full max-w-sm">
                                <div class="mb-1">
                                    ...
                                    <span v-if="row?.equipment?.description" class="text-xs font-medium text-slate-500 dark:text-slate-400 block mt-0.5 truncate">Model: {{ row?.equipment?.description }}</span>
                                </div>
                                <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                    <span v-if="row?.equipment?.brand" class="inline-flex items-center px-2 py-0.5 rounded text-[0.65rem] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700">
                                        {{ row?.equipment?.brand }}
                                    </span>
                                    <span v-if="row?.equipment_barcode" class="inline-flex items-center px-2 py-0.5 rounded text-[0.65rem] font-bold font-mono bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/30">
