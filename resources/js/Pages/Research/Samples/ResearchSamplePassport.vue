...
export default {
    ...
    computed: {
        monitoringRecords() {
            return Array.isArray(this.sample?.monitoring_records)
                ? this.sample.monitoring_records
                : [];
        },
        inventoryLogs() {
            return Array.isArray(this.sample?.inventory_logs)
                ? this.sample.inventory_logs
                : [];
        },
        metadataEntries() {
    ...
    <AppLayout :title="`Sample Passport ${sample.uid}`">
        ...
        <div class="max-w-7xl mx-auto p-6 space-y-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="group">
                                        ...
                                        <p class="mt-0.5 text-sm font-medium text-gray-900">
                                            <span v-if="sample.is_priority" class="inline-flex items-center gap-1 text-amber-600">
            ...
            <section class="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                ...
                <div class="p-6">
                    <!-- Breadcrumb Path -->
                    <div v-if="experimentPath.length" class="mb-5 flex items-center gap-2 text-sm flex-wrap">
                        <template v-for="(part, index) in experimentPath" :key="index">
                            <span v-if="index > 0" class="text-gray-300">/</span>
                            <span
                                class="rounded-md bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700"
                                :class="{'ring-1 ring-blue-200': index === experimentPath.length - 1}"
                            >
                                {{ part }}
                            </span>
                        </template>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            ...
                            <p class="mt-1 text-sm font-semibold text-gray-900">{{ sample.experiment?.study?.project?.title || "-" }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">Study</label>
                            <p class="mt-1 text-sm font-semibold text-gray-900">{{ sample.experiment?.study?.title || "-" }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">Experiment</label>
                            <p class="mt-1 text-sm font-semibold text-gray-900">{{ sample.experiment?.title || "-" }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">Generation</label>
                            <p class="mt-1 text-sm font-semibold text-gray-900">{{ sample.generation || "-" }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">PR Code</label>
                            <p class="mt-1 text-sm font-mono text-gray-900">{{ sample.pr_code || "-" }}</p>
                        </div>
                        <div class="rounded-lg border border-gray-100 bg-gray-50/30 p-4">
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">Line Label</label>
                            <p class="mt-1 text-sm font-semibold text-gray-900">{{ sample.line_label || "-" }}</p>
                        </div>
                    </div>
                </div>
            </section>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                    ...
                                    <span class="text-sm font-medium text-gray-900">{{ formatDate(sample.germination_date) }}</span>
                                </div>
                                <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                    <span class="text-sm text-gray-500">Sowing</span>
                                    <span class="text-sm font-medium text-gray-900">{{ formatDate(sample.sowing_date) }}</span>
                                </div>
                                <div class="flex items-center justify-between py-2 border-b border-gray-50">
                                    <span class="text-sm text-gray-500">Harvest</span>
                                    <span class="text-sm font-medium text-gray-900">{{ formatDate(sample.harvest_date) }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="rounded-lg bg-gray-50 p-3">
                                    ...
                                    <p class="text-lg font-semibold text-gray-900">{{ sample.plot_number || "-" }}</p>
                                </div>
                                <div class="rounded-lg bg-gray-50 p-3">
                                    <label class="text-xs text-gray-400 uppercase">Field</label>
                                    <p class="text-lg font-semibold text-gray-900">{{ sample.field_number || "-" }}</p>
                                </div>
                                <div class="rounded-lg bg-gray-50 p-3">
                                    <label class="text-xs text-gray-400 uppercase">Replication</label>
                                    <p class="text-lg font-semibold text-gray-900">{{ sample.replication_number || "-" }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <div class="flex items-center gap-6 text-xs text-gray-500">
                            <div class="flex items-center gap-1.5">
                                ...
                                Created by <span class="font-medium text-gray-700">{{ sample.creator?.name || "-" }}</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                Updated by <span class="font-medium text-gray-700">{{ sample.updater?.name || "-" }}</span>
                            </div>
                            <div class="ml-auto text-gray-400">
                                {{ formatDateTime(sample.created_at) }} → {{ formatDateTime(sample.updated_at) }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tr v-if="!monitoringRecords.length">
                            <td class="px-6 py-8 text-center text-gray-400" colspan="4">
                        ...
                        >
                            <td class="px-6 py-3.5 text-gray-900 font-medium whitespace-nowrap">{{ formatDate(record.recorded_on) }}</td>
                            <td class="px-6 py-3.5">
                                    <span class="inline-flex items-center rounded-md bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700">
                                        {{ record.stage || "-" }}
                                    </span>
                            </td>
                            <td class="px-6 py-3.5 text-gray-600">{{ record.recorder?.name || "-" }}</td>
                            <td class="px-6 py-3.5 text-gray-600 max-w-xs truncate" :title="record.notes">{{ record.notes || "-" }}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tr
                            ...
                            class="hover:bg-gray-50/50 transition-colors"
                        >
                            <td class="px-6 py-3.5 text-gray-900 whitespace-nowrap">{{ formatDateTime(log.created_at) }}</td>
                            <td class="px-6 py-3.5">
                                    <span class="inline-flex items-center rounded-md bg-rose-50 px-2 py-0.5 text-xs font-medium text-rose-700 capitalize">
                                        {{ log.action || "-" }}
                                    </span>
                            </td>
                            <td class="px-6 py-3.5 text-gray-600">{{ log.actor?.name || "System" }}</td>
                            <td class="px-6 py-3.5 text-gray-600">
                                <code class="text-xs bg-gray-100 px-1.5 py-0.5 rounded text-gray-700 break-all">{{ log.qr_payload || log.barcode_value || "-" }}</code>
                            </td>
                        </tr>
                        </tbody>
                    </table>
            ...
            >
                ...
                <div class="p-6">
                    <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        >
                            <label class="text-xs font-medium text-gray-400 uppercase tracking-wider">{{ entry[0] }}</label>
                            <p class="mt-1 text-sm font-medium text-gray-900 break-all">{{ toDisplay(entry[1]) }}</p>
                        </div>
                    </div>
                </div>
            </section>
