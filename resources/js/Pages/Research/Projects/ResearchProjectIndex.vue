    ...
    <div class="space-y-6 px-4 py-6">
        ...
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h2 class="mt-2 text-2xl font-semibold text-gray-900">
              Rice-first, commodity-flexible research tracking
            </h2>
            <p class="mt-3 max-w-3xl text-sm leading-6 text-gray-600">
              Use a consistent profile structure for projects, studies, experiments, field
              samples, lab handoffs, storage, and monitoring records.
            </p>
          </div>
          <div class="flex flex-wrap gap-2">
            <span
              v-for="commodity in (catalog.commodities || []).slice(0, 10)"
              :key="commodity"
      ...
      <section class="grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
        <div
          ...
          class="rounded-3xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500 lg:col-span-2 xl:col-span-3"
        >
          <LuFolderOpen class="mx-auto h-8 w-8 text-gray-400" />
          <p class="mt-3">
            No projects yet. Start by creating the research project profile, then build
            out studies and experiments.
          </p>
          <Link
            :href="route('research.projects.create')"
            class="mt-4 inline-flex rounded-lg bg-gray-900 px-4 py-2 text-xs font-semibold text-white hover:bg-black"
          >
            Create Project
          </Link>
        </div>
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <h2
                class="mt-2 text-xl font-semibold text-gray-900 group-hover:text-emerald-700"
              >
                {{ project.title }}
              </h2>
            </div>
          ...
          <div class="mt-5 space-y-2 text-sm text-gray-600">
            <p>
              <span class="font-medium text-gray-900">Commodity:</span>
              {{ project.commodity || "Generic / mixed" }}
            </p>
            <p>
              <span class="font-medium text-gray-900">Funding:</span>
              {{ project.funding_agency || "Not yet set" }}
            </p>
            <p>
              <span class="font-medium text-gray-900">Budget:</span>
              {{
                project.overall_budget
                  ? Number(project.overall_budget).toLocaleString()
                  : "Pending"
              }}
            </p>
          </div>
          ...
          <div class="mt-5 border-t border-gray-100 pt-4">
            ...
            <div
              ...
              class="mt-3 rounded-2xl bg-gray-50 px-4 py-3 text-sm text-gray-500"
            >
              Add the first study inside this project.
            </div>
            <div
              v-for="study in (project.studies || []).slice(0, 3)"
              :key="study.id"
              class="mt-3 rounded-2xl bg-gray-50 px-4 py-3"
            >
              <p class="text-sm font-medium text-gray-900">{{ study.title }}</p>
              <p class="mt-1 text-xs text-gray-500">
                {{ study.experiments_count }} experiments
              </p>
            </div>
          </div>
        </Link>
