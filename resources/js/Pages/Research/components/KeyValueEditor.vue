...
<template>
    <div class="space-y-3">
        ...
        <div
            ...
            :key="`${title}-${index}`"
            class="grid gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 md:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)_auto]">
            <div>
                <input
                    :value="row.key"
                    :id="`${title}-key-${index}`"
                ...
                <p
                    v-if="rowFieldError(index, 'key')"
                    class="mt-1 text-xs text-red-600">
                    {{ rowFieldError(index, "key") }}
                </p>
            </div>
            <div>
                <input
                ...
                <p
                    v-if="rowFieldError(index, 'value')"
                    class="mt-1 text-xs text-red-600">
                    {{ rowFieldError(index, "value") }}
                </p>
            </div>
            <button
                type="button"
