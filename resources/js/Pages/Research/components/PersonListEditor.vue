...
export default {
    ...
    methods: {
        ...
        addRow() {
            this.$emit("update:modelValue", [...this.normalizedRows(), { name: "", position: "" }]);
        },
        removeRow(index) {
            this.$emit(
                "update:modelValue",
...
<template>
    <div class="space-y-3">
        ...
        <div
            v-for="(person, index) in normalizedRows()"
            :key="`${title}-${index}`"
            class="grid gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 md:grid-cols-[1fr_1fr_auto]">
            <div>
                <input
                    :value="person.name"
                ...
                <p
                    v-if="rowFieldError(index, 'name')"
                    class="mt-1 text-xs text-red-600">
                    {{ rowFieldError(index, "name") }}
                </p>
            </div>
            <div>
                <input
                ...
                <p
                    v-if="rowFieldError(index, 'position')"
                    class="mt-1 text-xs text-red-600">
                    {{ rowFieldError(index, "position") }}
                </p>
            </div>
            <button
                type="button"
