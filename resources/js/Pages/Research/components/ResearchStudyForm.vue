...
export default {
    ...
    computed: {
        ...
        parentProjectRouteIdentifier() {
            return this.project?.route_identifier || this.project?.funding_code || this.project?.code || this.data?.project?.route_identifier || this.data?.project?.funding_code || this.data?.project?.code || this.parentProjectId;
        },
    ...
    methods: {
        initializeForm() {
            ...
            const payload = this.isEdit ? this.model.updateFields(this.data) : { ...this.model.createFields(), project_id: this.parentProjectId };
        ...
        async submitProxy() {
            ...
            if (response instanceof DtoResponse) {
                const studyIdentifier = this.model?.identifier(response?.data?.data || this.data || this.model)?.route ?? this.studyRouteIdentifier;
        ...
        <div class="space-y-6 p-6">
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select
                        v-model="form.study_leader_id"
                        class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <option value="">Select researcher...</option>
                        <option
                            v-for="opt in researchUserOptions"
                            :key="opt.value"
                            :value="opt.value">
                            {{ opt.label }}
                        </option>
                    ...
                    <p
                        v-if="fieldError('study_leader_id')"
                        class="mt-1 text-xs text-red-600">
                        {{ fieldError("study_leader_id") }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <p
                        v-if="fieldError('supervisor_id')"
                        class="mt-1 text-xs text-red-600">
                        {{ fieldError("supervisor_id") }}
                    </p>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <MultiSelectDropdown
                        ...
                        placeholder="Choose researchers..." />
                    <p
                        v-if="fieldError('staff_member_ids')"
                        class="mt-1 text-xs text-red-600">
                        {{ fieldError("staff_member_ids") }}
                    </p>
                </div>
            </div>
        </div>
