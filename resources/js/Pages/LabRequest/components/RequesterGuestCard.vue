...
export default {
    ...
    methods: {
        ...
        syncResourceSelections() {
            const equipmentSelections = this.mergeUnique(this.toArray(this.form?.equipments_to_use), this.toArray(this.form?.ict_equipments), this.toArray(this.form?.laboratory_equipments), this.toArray(this.form?.biofreezers), this.toArray(this.form?.medicool_units), this.toArray(this.form?.plant_growth_chambers));

            const laboratorySelections = this.mergeUnique(this.toArray(this.form?.labs_to_use), this.toArray(this.form?.laboratory_access), this.toArray(this.form?.field_spaces), this.toArray(this.form?.screenhouse_spaces), this.toArray(this.form?.office_spaces), this.toArray(this.form?.storage_spaces), this.toArray(this.form?.utility_spaces), this.toArray(this.form?.parking_spaces));

            const consumableSelections = this.mergeUnique(this.toArray(this.form?.consumables_to_use), this.toArray(this.form?.ict_supplies), this.toArray(this.form?.laboratory_consumables), this.toArray(this.form?.office_supplies), this.toArray(this.form?.iec_materials), this.toArray(this.form?.tokens));
    ...
    computed: {
        requestTypeWhitelist() {
            const defaults = ["Biofreezer", "Field Experimental Space", "ICT Equipment", "ICT Supplies", "IEC Materials", "Laboratory Access", "Laboratory Consumables", "Laboratory Equipment", "Medicool", "Office Space", "Office Supplies", "Parking Space", "Plant Growth Chamber", "Screenhouse Space", "Storage Space", "Tokens", "Utility Space"];

            const fromProps = (this.requestTypeOptions || []).map((entry) => (typeof entry === "string" ? { name: entry, label: entry } : { name: entry?.name ?? entry?.label, label: entry?.label ?? entry?.name })).filter((entry) => entry.name);

            const seen = new Set(fromProps.map((entry) => entry.name));
            const missing = defaults.filter((name) => !seen.has(name)).map((name) => ({ name, label: name }));
        ...
        <div class="border-b border-gray-100 bg-slate-50/50 p-6 dark:border-slate-800 dark:bg-slate-800/50">
            <div class="mx-auto max-w-2xl overflow-x-auto">
                <ProgressTabs
                    :steps="stepLabels"
                    :current="currentStep"
                    @update:current="handleStepChange"
                    :clickable="true" />
            </div>
        </div>
        <form
                    ...
                    <div class="relative w-full max-w-2xl">
                        <h2 class="mb-2 flex items-center justify-between">
                            ...
                            <transition-container type="slide-bottom">
                                <InputError
                                    v-show="!!hasErr('request_type')"
                                    :message="errMsg('request_type')" />
                ...
                <div
                    ...
                    <div class="flex max-w-2xl flex-col gap-5">
                        ...
                        <TextInput
                            ...
                            label="Special Request or Instructions"
                        ...
                        <TextInput
                            ...
                            label="Project Title"
                            placeholder="Research or Thesis Title"
                        ...
                        <div class="grid grid-cols-2 gap-5">
                            ...
                            <TimeInput
                                ...
                                required
                                type="text"
                                :error="errMsg('time_of_use')"
                                label="Time of Use"
                                autocomplete="time_of_use"
                                @input="form.clearErrors('time_of_use')" />
                        </div>
                        <div
                            v-if="requiresEndTime"
                            class="grid grid-cols-2 gap-5">
                            <DateInput
                                id="date_of_use_end"
                                v-model="form.date_of_use_end"
                                required
                                type="text"
                                :error="errMsg('date_of_use_end')"
                                label="End Date of Use"
                                autocomplete="date_of_use_end"
                                @input="form.clearErrors('date_of_use_end')" />
                            <TimeInput
                                id="time_of_use_end"
                                v-model="form.time_of_use_end"
                                required
                                type="text"
                                :error="errMsg('time_of_use_end')"
                                label="End Time of Use"
                                autocomplete="time_of_use_end"
                                @input="form.clearErrors('time_of_use_end')" />
                        </div>
                    </div>
                </div>
            </transition>
