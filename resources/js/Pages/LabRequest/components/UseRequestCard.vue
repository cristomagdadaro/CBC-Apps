...
import {
    X, Calendar, Clock, User, Building2, Briefcase, FileText,
    FlaskConical, Package, Microscope, AlertCircle, CheckCircle2,
...
<template>
    ...
    >
        ...
        <div class="absolute left-0 top-0 bottom-0 w-1.5 transition-colors duration-300 group-hover:w-2" :class="statusConfig.border"></div>
        ...
        <div class="p-5 pl-6 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            ...
            <div class="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2.5 shrink-0 mt-2 sm:mt-0 w-full sm:w-auto">
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest border shadow-sm"
                ...
                <div class="flex flex-col items-end gap-0.5 text-right">
                    ...
                    <span v-if="lifecycleHint" class="text-[0.65rem] font-bold" :class="formsData?.is_overdue ? 'text-orange-500 dark:text-orange-400' : 'text-slate-500 dark:text-slate-400'">
                        {{ lifecycleHint }}
                    </span>
                </div>
            </div>
            ...
            <div class="absolute right-5 bottom-4 hidden sm:flex items-center text-[0.65rem] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-0 translate-x-2">
                Review <ChevronRight class="w-3.5 h-3.5 ml-0.5" />
            </div>
        </div>
    </div>
            ...
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 flex items-start justify-between backdrop-blur-md sticky top-0 z-20">
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        ...
                        <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1.5">{{ formsData.id }}</p>
                    </div>
                </div>
                <button @click="closeModal" class="p-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors text-slate-500 dark:text-slate-400">
                    <X class="w-5 h-5" />
                </button>
            </div>
            ...
            <div class="p-6 space-y-8 max-h-[75vh] overflow-y-auto custom-scrollbar bg-slate-50/50 dark:bg-slate-900/50">
                ...
                <div class="space-y-4">
                    <h4 class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Clock class="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> Request Timeline
                    </h4>
                    ...
                    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden">
                        <!-- Connecting Line -->
                        <div class="absolute left-9 top-8 bottom-8 w-0.5 bg-slate-200 dark:bg-slate-700" aria-hidden="true"></div>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ formatDate(formsData.created_at) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="formsData.approved_at" class="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    <p class="font-bold text-slate-900 dark:text-white text-sm">Approved <span v-if="formsData.approved_by" class="text-slate-400 font-medium ml-1">by {{ formsData.approved_by }}</span></p>
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ formatDate(formsData.approved_at) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="formsData.released_at" class="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    <p class="font-bold text-slate-900 dark:text-white text-sm">Released <span v-if="formsData.released_by" class="text-slate-400 font-medium ml-1">by {{ formsData.released_by }}</span></p>
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ formatDate(formsData.released_at) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="formsData.returned_at" class="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    <p class="font-bold text-slate-900 dark:text-white text-sm">Returned <span v-if="formsData.returned_by" class="text-slate-400 font-medium ml-1">by {{ formsData.returned_by }}</span></p>
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ formatDate(formsData.returned_at) }}</p>
                                </div>
                            </div>
                            ...
                            <div v-if="formsData.request_status === 'rejected'" class="flex items-start gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ formatDate(formsData.updated_at) }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <h4 class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            <User class="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> Requester
                        </h4>
                        <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 space-y-3">
                            <div>
                                <p class="text-base font-black text-slate-900 dark:text-white tracking-tight">{{ requesterDisplayName }}</p>
                                <p class="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-wider">{{ displayText(formsData.requester?.position) }}</p>
                            </div>
                            <div class="pt-3 border-t border-slate-100 dark:border-slate-700/50">
                                <p class="text-sm font-medium text-slate-600 dark:text-slate-300 leading-relaxed">{{ displayText(formsData.requester?.affiliation) }}</p>
                                <p v-if="formsData.requester?.philrice_id" class="text-xs font-bold text-indigo-600 dark:text-indigo-400 mt-1.5 uppercase tracking-wide">ID: {{ formsData.requester.philrice_id }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <h4 class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                            <Clock class="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> Schedule
                        </h4>
                        <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 space-y-4">
                            <div>
                                ...
                                <p class="text-sm font-bold text-slate-800 dark:text-slate-200">
                                    ...
                                    <span class="text-indigo-600 dark:text-indigo-400">{{ formatTime(formsData.requestForm?.time_of_use) }}</span>
                                </p>
                            </div>
                            <div v-if="formsData.requestForm?.date_of_use_end" class="pt-4 border-t border-slate-100 dark:border-slate-700/50">
                                ...
                                <p class="text-sm font-bold text-slate-800 dark:text-slate-200">
                                    ...
                                    <span class="text-slate-400 mx-1">·</span>
                                    <span class="text-indigo-600 dark:text-indigo-400">{{ formatTime(formsData.requestForm?.time_of_use_end) }}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="space-y-4">
                    <h4 class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Info class="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> Other Information
                    </h4>
                    <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 space-y-4">
                        <div v-if="formsData.requestForm?.project_title">
                            ...
                            <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ displayText(formsData.requestForm?.project_title) }}</p>
                        </div>
                        <div v-if="formsData.requestForm?.request_purpose">
                            ...
                            <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ displayText(formsData.requestForm?.request_purpose) }}</p>
                        </div>
                        <div v-if="formsData.requestForm?.request_details">
                            ...
                            <p class="text-sm font-medium text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{{ displayText(formsData.requestForm.request_details) }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="hasItems" class="space-y-4">
                    <h4 class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <Package class="w-4 h-4 text-indigo-500 dark:text-indigo-400" /> Requested Items
                    </h4>
