    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 p-3 sm:p-5 gap-4 text-slate-900 dark:text-slate-100">
        <div class="flex flex-col col-span-1 sm:col-span-2 gap-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xs">
            <div
                ...
                <div class="grid grid-cols-3 gap-2 w-full bg-slate-50/60 dark:bg-slate-800/40 p-4 rounded-xl">
                    <div class="flex flex-col text-center justify-center p-1">
                        ...
                        <span class="text-[0.68rem] uppercase font-bold tracking-wider text-slate-500 dark:text-slate-400 mt-1">REMAINING</span>
                    </div>
                    <div class="flex flex-col text-center justify-center p-1">
                        <span class="text-xl font-bold text-slate-900 dark:text-slate-100">
                            {{ formatNumber(displayData.total_outgoing) }}
                        </span>
                        <span class="text-[0.68rem] uppercase font-bold tracking-wider text-slate-500 dark:text-slate-400 mt-1">CONSUMED</span>
                    </div>
                    <div class="flex flex-col text-center justify-center p-1">
                        <span class="text-xl font-bold text-lime-600 dark:text-lime-400">{{ utilizationPercentage }}%</span>
                        <span class="text-[0.68rem] uppercase font-bold tracking-wider text-slate-500 dark:text-slate-400 mt-1">UTILIZATION</span>
                    </div>
                </div>
            </div>
            <div class="flex flex-col gap-3 pt-2">
                <transaction-report-accordion
                    ...
                    class="w-full mb-3"
                    :reports="reportsList" />
                <h1 class="text-sm sm:text-base font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 sm:p-5 h-fit shadow-xs">
            ...
            <div class="flex flex-col text-xs gap-2 w-full">
                <div
                    v-if="processing"
                    class="text-center py-4 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-500 font-semibold">
                ...
                <div
                    ...
                    :key="transaction.id"
                    class="p-3 rounded-xl border flex flex-col gap-1.5 transition-colors"
                    :class="transaction.transac_type === 'incoming' ? 'bg-emerald-50/60 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-900/60' : 'bg-rose-50/60 dark:bg-rose-950/30 border-rose-200 dark:border-rose-900/60'">
                    <div class="flex justify-between items-center leading-tight">
                        <div>
                            <p class="font-bold text-slate-900 dark:text-slate-100">
                                {{ transaction.actor_display_name || "Unknown" }}
                            </p>
                            <p class="text-[0.68rem] text-slate-500 dark:text-slate-400 mt-0.5">
                                {{ formatDate(transaction.created_at) }}
                            </p>
                        </div>

                        <span
                            class="font-extrabold text-xs px-2 py-1 rounded-lg"
                    ...
                    <div
                        ...
                        class="border-t pt-1.5 border-slate-200/60 dark:border-slate-800 text-[0.7rem] text-slate-600 dark:text-slate-300">
