...
<template>
    <div
        ...
        class="grid w-full grid-cols-1 gap-2 text-slate-900 sm:grid-cols-[repeat(auto-fill,minmax(300px,1fr))] sm:gap-3 dark:text-slate-100">
        <div
            ...
            <div class="flex h-full items-start justify-between gap-3">
                <div class="flex min-w-0 flex-col space-y-1 text-[0.7rem] leading-none text-slate-500 sm:text-xs dark:text-slate-400">
                    ...
                    <!-- Barcode -->
                    <span :class="item.barcode ? 'text-slate-500 dark:text-slate-400' : 'font-bold text-rose-500'">
                ...
                <!-- Quantity badge -->
                <div class="my-auto shrink-0 text-right">
                    <span class="px-2 py-1 text-xs font-extrabold text-slate-800 dark:text-slate-200">{{ formatNumber(item.remaining_quantity) }} {{ item.unit || "" }}</span>
                </div>
            </div>
        </div>
    </div>
