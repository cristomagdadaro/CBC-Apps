...
export default {
    ...
    methods: {
        ...
        verifiedClass(registration) {
            return registration.is_email_verified ? "border-emerald-200 dark:border-emerald-800/80 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300" : "border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 text-slate-600 dark:text-slate-400";
        },
        ...
        async approveRegistration(registration, force = false) {
            ...
            try {
                await this.fetchPutApi(PersonnelRegistration.endpoints.status, registration.id, {
                    status: "approved",
    ...
    <AppLayout>
        ...
        <div class="default-container pt-5">
            <form
                ...
                @submit.prevent="searchRegistrations">
                <div class="grid w-full grid-rows-2">
                    ...
                    <div
                        ...
                        <div class="flex w-full items-center justify-center gap-1">
                            ...
                            <div class="flex flex-col whitespace-nowrap text-center text-xs font-semibold text-slate-600 dark:text-slate-400">
                                <span
                                    class="mx-1 font-medium"
                                    title="current page and total pages">
                                    <span>{{ registrationsFromApi?.current_page }}</span>
                                    /
                                    <span>{{ registrationsFromApi?.last_page }}</span>
                                </span>
                            </div>
                            <paginate-btn
                                @click="
                                    form.page = Math.min(registrationsFromApi?.last_page, form.page + 1);
                                    searchRegistrations();
                                "
                            ...
                            <paginate-btn
                                @click="
                                    form.page = registrationsFromApi?.last_page;
                                    searchRegistrations();
                                "
                                :disabled="form.page === registrationsFromApi?.last_page">
                                Last
            ...
            <div class="mt-3 overflow-hidden rounded-2xl">
                <div
                    v-if="registrationRows.length && !processing"
                    class="grid gap-4 p-1 lg:grid-cols-2">
                    <article
                        v-for="registration in registrationRows"
                        :key="registration.id"
                        class="shadow-xs rounded-2xl border border-slate-200 bg-white p-5 transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-base font-bold text-slate-900 sm:text-lg dark:text-slate-100">
                                    {{ registration.full_name }}
                                </h3>
                                <p class="text-xs font-medium text-slate-500 sm:text-sm dark:text-slate-400">
                                    {{ registration.position || "No position supplied" }}
                                </p>
                            ...
                            <div class="flex flex-col items-end gap-1">
                                <span
                                    class="rounded-full border px-3 py-1 text-[11px] font-bold uppercase"
                                    :class="statusClass(registration.status)">
                                    {{ registration.status }}
                                </span>
                                <span
                                    class="rounded-full border px-3 py-1 text-[11px] font-semibold"
                                    :class="verifiedClass(registration)">
                        ...
                        <p
                            v-if="registration.rejection_remarks"
                            class="mt-4 rounded-xl bg-rose-50 p-3 text-sm text-rose-800">
                        ...
                        <div
                            ...
                            <div
                                v-if="rejectingId === registration.id"
                                class="space-y-2">
                                <textarea
                                ...
                                <div class="flex justify-end gap-2">
                                    <button
                                        type="button"
                                        class="rounded-lg border px-3 py-2 text-sm font-semibold text-gray-700"
                                        @click="cancelReject">
                                        Cancel
                                    </button>
                                    <button
                                        ...
                                        :disabled="!rejectionRemarks.trim() || processing"
                            ...
                            <div
                                ...
                                <button
                                    ...
                                    @click="beginReject(registration)">
                                    Reject
                                </button>
                                <button
                ...
                <div
                    ...
                    class="shadow-xs rounded-2xl border border-slate-200 bg-white py-8 text-center text-xs font-semibold text-slate-500 sm:text-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                    Searching...
                </div>

                <div
                    v-else-if="registrationsFromApi && registrationsFromApi.total === 0 && hasSearchTerm"
                    class="shadow-xs rounded-2xl border border-slate-200 bg-white py-8 text-center text-xs font-semibold text-slate-500 sm:text-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
            ...
            <div
                v-if="registrationsFromApi && registrationsFromApi.data?.length"
        ...
        <ConfirmationModal
            ...
            @close="cancelBypass">
            <template #title>Bypass Email Verification?</template>
            <template #content>
                The email (
                <strong>{{ bypassingRegistration?.email }}</strong>
            ...
            <template #footer>
                <button
                    type="button"
                    class="mr-2 rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700"
                    @click="cancelBypass">
                    Cancel
                </button>
                <button
                    type="button"
                    class="rounded-lg bg-amber-600 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50"
                    :disabled="processing"
                    @click="executeBypass">
