                ...
                <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
                    <div class="space-y-4 flex-1">
                        <div>
                            ...
                            <h1 class="mt-2 text-xl sm:text-2xl font-semibold text-slate-900 dark:text-slate-100">{{ item.name }}</h1>
                            <p class="mt-1 text-xs sm:text-sm text-slate-600 dark:text-slate-400">{{ item.category?.name || 'Uncategorized' }}</p>
                        </div>
                        ...
                        <div class="grid gap-3 text-xs sm:text-sm text-slate-600 dark:text-slate-300 md:grid-cols-2">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <span class="ml-1 line-clamp-2" :title="item.description">{{ item.description || 'N/A' }}</span>
                            </div>
                            <div class="md:col-span-2">
                                <span class="font-medium text-slate-900 dark:text-slate-100 block mb-1">Specifications:</span>
                                <span class="whitespace-pre-wrap block p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800">{{ item.specifications || 'N/A' }}</span>
                            </div>
                        </div>
                    </div>

                    <div v-if="item.image" class="w-full sm:w-[24rem] shrink-0">
                        <img :src="item.image" alt="Item Image" class="rounded-xl object-cover w-full max-h-64 border border-slate-200 dark:border-slate-700 shadow-sm">
                    </div>
                </div>
            </section>
                ...
                >
                    ...
                    <template #cell-itemWithPrriCode="{ row }">
                        <div class="flex flex-col leading-tight gap-0.5">
                            ...
                            <span v-if="row.item?.brand" class="text-[0.70rem] text-slate-500 dark:text-slate-400">
                                {{ row.item.brand }}
                            </span>
                            <span v-if="row.barcode" class="text-[0.70rem] text-slate-500 dark:text-slate-400">
                                {{ row.barcode }}
                            </span>
                        </div>
                    </template>
                    <template #cell-actorWithRemarks="{ row }">
                        <div class="flex flex-col">
                            ...
                            <span v-if="row.remarks" class="text-xs text-slate-500 mt-0.5">
                                {{ row.remarks }}
                            </span>
                        </div>
                    </template>
