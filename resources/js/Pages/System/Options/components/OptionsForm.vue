...
<template>
    <form @submit.prevent="submitProxy" class="space-y-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-800 p-6 md:p-8">
        ...
        <div class="bg-slate-50/50 dark:bg-slate-800/30 border border-slate-200/60 dark:border-slate-700/60 rounded-xl p-5 shadow-sm">
            <label for="value" class="block text-[0.65rem] font-semibold uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-3 flex items-center gap-1.5">
                <Settings2 class="w-3.5 h-3.5" /> Value <span class="text-rose-500 ml-0.5">*</span>
            </label>
            ...
            <p v-if="form.errors?.value" class="mt-2 text-xs font-semibold text-rose-600 dark:text-rose-400">
                {{ form.errors.value }}
            </p>
            <div v-else-if="selectOptionsEmpty" class="mt-2.5 flex items-center gap-1.5 text-amber-600 dark:text-amber-400">
                <AlertTriangle class="w-3.5 h-3.5 shrink-0" />
                <p class="text-[0.65rem] font-bold uppercase tracking-widest">
        ...
        <div v-if="form.type === 'select'" class="bg-indigo-50/50 dark:bg-indigo-500/5 border border-indigo-100 dark:border-indigo-500/20 rounded-xl p-5 shadow-sm">
            <label for="options-metadata" class="block text-[0.65rem] font-semibold uppercase tracking-widest text-indigo-600 dark:text-indigo-400 mb-3 flex items-center gap-1.5">
                <ListPlus class="w-3.5 h-3.5" /> Select Choices
            </label>
            
            <div id="options-metadata" class="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/50 p-4 shadow-sm">
                <component :is="SelectOptionsEditorOptions" v-model="form.options" />
            </div>
            
            <p v-if="form.errors?.options" class="mt-2 text-xs font-semibold text-rose-600 dark:text-rose-400">
                {{ form.errors.options }}
            </p>
            <p class="mt-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 leading-relaxed">
