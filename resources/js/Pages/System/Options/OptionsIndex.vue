    ...
    methods: {
        ...
        moduleStatusBanner(item) {
            ...
            if (mode === "maintenance") {
                return {
                    className:
                        "border-amber-200 bg-amber-50/80 text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300",
            ...
            if (mode === "deactivated") {
                return {
                    className:
                        "border-rose-200 bg-rose-50/80 text-rose-800 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-300",
            ...
            <section class="transition-colors">
                ...
                <TabNavigation v-model="activeTab" :tabs="optionTabs">
                    <template #default="{ activeKey }">
                        ...
                        <!-- CONTROLS TAB -->
                        <div v-if="activeKey === 'controls'" class="space-y-6 pt-4">
                            ...
                            <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/60 dark:border-slate-800 p-5 md:p-7">
                                <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6 pb-5 border-b border-slate-100 dark:border-slate-800/60">
                                    ...
                                    >
                                        <Loader2 v-if="workflowToggleSaving" class="w-4 h-4 animate-spin"/>
                                        <Save v-else class="w-4 h-4"/>
                                        <span>{{ workflowToggleSaving ? "Saving..." : "Save Changes" }}</span>
                                    </button>
                                </div>

                                <div v-if="workflowToggleLoading" class="flex flex-col items-center justify-center py-10 text-slate-500 dark:text-slate-400">
                                        ...
                                        <div class="flex items-center gap-3.5">
                                            <div
                                                ...
                                                :class="workflowToggles.event_workflow_enabled
                                                    ? 'bg-white dark:bg-slate-800 border-indigo-200 dark:border-indigo-500/30 text-indigo-600 dark:text-indigo-400'
                                                    : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500'"
                                            >
                                                <CheckCircle2 v-if="workflowToggles.event_workflow_enabled" class="w-5 h-5"/>
                                        ...
                                        <div class="flex items-center gap-3.5">
                                            <div
                                                ...
                                                :class="workflowToggles.participant_workflow_enabled
                                                    ...
                                                    : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500'"
                                            >
                                                <CheckCircle2 v-if="workflowToggles.participant_workflow_enabled" class="w-5 h-5"/>
                                                <XCircle v-else class="w-5 h-5"/>
                                            </div>
                                            <div>
                                        ...
                                        <div class="flex items-center gap-3.5">
                                            <div
                                                ...
                                                :class="workflowToggles.participant_verification_enabled
                                                    ...
                                                    : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500'"
                                            >
                                                <CheckCircle2 v-if="workflowToggles.participant_verification_enabled" class="w-5 h-5"/>
                                                <XCircle v-else class="w-5 h-5"/>
                                            </div>
                                            <div>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6 pb-5 border-b border-slate-100 dark:border-slate-800/60">
                                    <div class="flex items-start gap-3.5">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed max-w-xl">
                                            ...
                                            <div class="mt-3 flex flex-wrap gap-2 text-[0.65rem] font-mono font-semibold">
                                                <span class="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 flex items-center gap-1.5 shadow-sm">
                                                    <Server class="w-3 h-3" /> Local: {{ deploymentAccessMeta.local_url }}
                                                </span>
                                                <span class="px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 flex items-center gap-1.5 shadow-sm">
                                                    <Globe class="w-3 h-3" /> Internet: {{ deploymentAccessMeta.internet_url }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    >
                                        <Loader2 v-if="deploymentAccessSaving" class="w-4 h-4 animate-spin"/>
                                        <Save v-else class="w-4 h-4"/>
                                        <span>{{ deploymentAccessSaving ? "Saving..." : "Save Module Rules" }}</span>
                                    </button>
                                </div>

                                <div v-if="deploymentAccessLoading" class="flex flex-col items-center justify-center py-10 text-slate-500 dark:text-slate-400">
