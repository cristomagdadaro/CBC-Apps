...
<template>
    <div @click="showModal = true" :class="[
        ...
        <div :class="['absolute left-0 top-0 h-full w-1', statusConfig.bg.replace('bg-', 'bg-').replace('50', '500')]">
        </div>
            ...
            <div class="flex flex-col w-full">
                <div v-if="canApprove" class="flex flex-col gap-2">
                    <custom-dropdown label="Assign Vehicle" required searchable :withAllOption="false"
                        ...
                        <template #icon="{ open }">
                            <LuChevronRight :class="open ? 'rotate-90' : 'rotate-90'" class="h-4 w-4 duration-300" />
                        </template>
                    </custom-dropdown>
