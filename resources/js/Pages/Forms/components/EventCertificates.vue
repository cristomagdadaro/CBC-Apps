            ...
            <div class="p-6 space-y-8">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800/50">
                        ...
                        <label class="inline-flex items-center gap-2 cursor-pointer bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm transition-all hover:bg-slate-100 dark:hover:bg-slate-800" :class="{ 'opacity-50 cursor-not-allowed': !hasSavedTemplate }">
                            <input 
                    ...
                    <div v-if="useSavedTemplate && hasSavedTemplate" class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 dark:border-indigo-500/20 rounded-xl shadow-sm">
                    ...
                    >
                        <input type="file" accept=".pptx" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" @change="onFileInput('template', $event)" />
                        
                        <div v-if="!templateFile" class="space-y-3 pointer-events-none">
                            <div class="mx-auto w-14 h-14 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-sm transition-transform duration-300" :class="{'scale-110 border-indigo-300 dark:border-indigo-500': dragOver.template}">
                                <LuUploadCloud class="w-6 h-6 text-slate-400 dark:text-slate-500" :class="{'text-indigo-500 dark:text-indigo-400': dragOver.template}" />
                            </div>
                            <div>
                        ...
                        <div v-else class="flex flex-col sm:flex-row items-center justify-center gap-3">
                            <div class="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl shadow-sm pointer-events-auto">
                                ...
                                <span class="text-sm font-bold text-emerald-800 dark:text-emerald-300">{{ templateFile.name }}</span>
                                <div class="w-px h-4 bg-emerald-200 dark:bg-emerald-500/30 mx-1"></div>
                                <button @click.stop="templateFile = null" class="p-1 hover:bg-emerald-200 dark:hover:bg-emerald-500/30 rounded-md transition-colors">
                ...
                <div class="space-y-4">
                    ...
                    <div v-if="useEventData" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/60 rounded-xl p-4 sm:p-5 shadow-sm">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select v-model="selectedEmailColumn" class="block w-full pl-3 pr-10 py-2.5 text-sm font-semibold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg shadow-sm appearance-none transition-colors">
                                        <option value="" disabled>Select column</option>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="relative">
                                    <select v-model="selectedSubformType" class="block w-full pl-3 pr-10 py-2.5 text-sm font-semibold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg shadow-sm appearance-none transition-colors">
                        ...
                        <div class="border border-slate-200 dark:border-slate-700 rounded-xl  shadow-sm bg-white dark:bg-slate-900">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-5 py-4 bg-slate-50/80 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                                <div class="text-sm font-bold text-slate-800 dark:text-slate-200 flex items-center gap-2">
                                    <span class="inline-flex items-center justify-center bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded-full text-xs font-black">{{ selectedRecipientCount }}</span>
                                    Possible Recipients
                                </div>
                                <label class="inline-flex items-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 cursor-pointer shadow-sm">
                                    <input type="checkbox" :checked="allFilteredRecipientsSelected" @change="toggleAllFilteredRecipients" class="w-4 h-4 text-indigo-600 rounded border-slate-300 dark:border-slate-600 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-900" />
                                    Select all shown
                                </label>
                            </div>
                            ...
                            <div v-if="filteredRecipients.length">
                                ...
                                >
                                    <template #cell-select="{ row: recipient }">
                                        <input v-model="selectedRecipientIds" :value="recipient.id" type="checkbox" class="w-4 h-4 text-indigo-600 rounded border-slate-300 dark:border-slate-600 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-800" />
                                    </template>
                                    
                                    <template #cell-name="{ row: recipient }">
                                        <span class="font-semibold text-slate-800 dark:text-slate-200 truncate max-w-[150px] inline-block align-bottom">{{ recipientName(recipient) }}</span>
                                    </template>
                                    
                                    <template #cell-email="{ row: recipient }">
                                        <span class="font-medium text-slate-600 dark:text-slate-400 truncate max-w-[180px] inline-block align-bottom">{{ recipientEmail(recipient) }}</span>
                                    </template>
                                    
                                    <template #cell-status="{ row: recipient }">
                                        <div class="flex flex-col gap-1 w-fit">
                                            <span class="inline-flex w-fit items-center border rounded-full px-2.5 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider" :class="recipientCertificateStatusConfig(recipient).badge">
                                                {{ recipientCertificateStatusConfig(recipient).label }}
                                            </span>
                                            <span v-if="recipientCertificateStatus(recipient) === 'sent' && formatCertificateSentAt(recipient)" class="text-[0.65rem] font-semibold text-slate-500 dark:text-slate-500 ml-1">
                                                {{ formatCertificateSentAt(recipient) }}
                                            </span>
                                        </div>
                                    </template>
                                    ...
                                    <template #cell-subform="{ row: recipient }">
                                        <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">{{ recipient.subform_type || '—' }}</span>
                                    </template>
                                </DataTable>
                            </div>
                            <div v-else class="px-6 py-12 text-sm font-medium text-slate-400 dark:text-slate-500 text-center flex flex-col items-center">
                        ...
                        <div v-if="!loadingColumns && responseColumns.length === 0" class="flex items-center gap-2 p-4 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 text-sm font-bold text-amber-700 dark:text-amber-400 shadow-sm">
                            <LuAlertTriangle class="w-5 h-5 shrink-0" /> No response data found for this event.
                        </div>
                    </div>
                    ...
                    >
                        <input type="file" accept=".xlsx,.csv" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" @change="onFileInput('data', $event)" />
                        
                        <div v-if="!dataFile" class="space-y-3 pointer-events-none">
                            <div class="mx-auto w-14 h-14 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full flex items-center justify-center shadow-sm transition-transform duration-300" :class="{'scale-110 border-indigo-300 dark:border-indigo-500': dragOver.data}">
                                <LuSheet class="w-6 h-6 text-slate-400 dark:text-slate-500" :class="{'text-indigo-500 dark:text-indigo-400': dragOver.data}" />
                            </div>
                            <div>
                        ...
                        <div v-else class="flex flex-col sm:flex-row items-center justify-center gap-3">
                            <div class="flex items-center gap-2 p-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl shadow-sm pointer-events-auto">
                                ...
                                <span class="text-sm font-bold text-emerald-800 dark:text-emerald-300">{{ dataFile.name }}</span>
                                <div class="w-px h-4 bg-emerald-200 dark:bg-emerald-500/30 mx-1"></div>
                                <button @click.stop="dataFile = null" class="p-1 hover:bg-emerald-200 dark:hover:bg-emerald-500/30 rounded-md transition-colors">
                                    <LuX class="w-4 h-4 text-emerald-700 dark:text-emerald-400" />
                                </button>
                            </div>
                        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-200 dark:border-slate-700/60 shadow-sm">
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select v-model="outputFormat" class="block w-full pl-3 pr-10 py-2.5 text-sm font-semibold border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 rounded-lg shadow-sm appearance-none transition-colors">
                ...
                <!-- Progress Bar -->
                <div v-if="uploading" class="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm">
                    <div class="flex justify-between text-xs font-bold uppercase tracking-wider mb-2">
                        <span class="text-slate-500 dark:text-slate-400 flex items-center gap-1.5"><LuUploadCloud class="w-4 h-4" /> Uploading files...</span>
                    ...
                    <div class="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2 overflow-hidden shadow-inner">
                        <div class="bg-indigo-600 dark:bg-indigo-500 h-full rounded-full transition-all duration-300 ease-out" :style="{ width: `${uploadProgress}%` }"></div>
                    </div>
                </div>

                <!-- Server Status Panel -->
                <div v-if="processingOnServer || serverStatus !== 'idle'" class="flex items-center gap-4 p-5 rounded-xl border shadow-sm transition-all" :class="{
                    ...
                    <div class="p-2 rounded-lg bg-white/50 dark:bg-slate-900/50 shadow-sm shrink-0" :class="`text-${statusConfig.color}-600 dark:text-${statusConfig.color}-400`">
                        <component :is="statusConfig.icon" class="w-6 h-6" :class="{ 'animate-spin': statusConfig.spin }" />
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-bold tracking-wide" :class="`text-${statusConfig.color}-900 dark:text-${statusConfig.color}-200`">{{ statusConfig.text }}</p>
                        <p v-if="message" class="text-xs font-semibold mt-0.5 truncate" :class="`text-${statusConfig.color}-700 dark:text-${statusConfig.color}-400/80`">{{ message }}</p>
                    </div>
                </div>

                <!-- Alerts -->
                <TransitionGroup name="alert" tag="div" class="space-y-3">
                    <div v-if="errorMessage" key="error" class="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl shadow-sm">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm font-medium text-red-700 dark:text-red-300 mt-0.5 leading-relaxed">{{ errorMessage }}</p>
                        </div>
                        <button @click="errorMessage = ''" class="p-1 hover:bg-red-100 dark:hover:bg-red-500/20 rounded-md transition-colors text-red-500">
                            <LuX class="w-4 h-4" />
                        </button>
                    </div>

                    <div v-if="message && !processingOnServer && serverStatus === 'completed'" key="success" class="flex items-start gap-3 p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 rounded-xl shadow-sm">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-sm font-medium text-emerald-700 dark:text-emerald-300 mt-0.5 leading-relaxed">{{ message }}</p>
                        </div>
                        <button @click="message = ''" class="p-1 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 rounded-md transition-colors text-emerald-500">
                            <LuX class="w-4 h-4" />
                        </button>
                    </div>
                </TransitionGroup>
            ...
            <div class="p-6">
                <ul class="space-y-4 text-sm font-medium text-slate-600 dark:text-slate-300">
                    <li class="flex items-start gap-3">
                        ...
                        <span>Use placeholder format <code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-indigo-600 dark:text-indigo-400 font-mono text-xs font-bold">&lt;&lt;COLUMN_NAME&gt;&gt;</code> in text boxes.</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <LuCheck class="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                        <span>Match column names exactly from your data source (e.g., <code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-indigo-600 dark:text-indigo-400 font-mono text-xs font-bold">fullname_8647</code>).</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <LuCheck class="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                        <span class="leading-relaxed">You can use built-in system placeholders <code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-indigo-600 dark:text-indigo-400 font-mono text-xs font-bold">&lt;&lt;EVENT_TITLE&gt;&gt;</code>, <code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-indigo-600 dark:text-indigo-400 font-mono text-xs font-bold">&lt;&lt;EVENT_DATE&gt;&gt;</code>, and <code class="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-indigo-600 dark:text-indigo-400 font-mono text-xs font-bold">&lt;&lt;DATE_GIVEN&gt;&gt;</code>.</span>
                    </li>
                    <li class="flex items-start gap-3">
                        <LuCheck class="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                    ...
                    <li class="flex items-start gap-3">
                        <LuCheck class="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" />
                        <span>Save your template explicitly in <strong>.pptx</strong> format only.</span>
                    </li>
                </ul>
            </div>
        </div>
