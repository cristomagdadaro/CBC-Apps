...
<template>
    <div class="w-full max-w-3xl mx-auto space-y-5">
        ...
        <!-- Error Alert -->
        <div v-if="error" class="flex items-center gap-2 text-sm font-semibold text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 px-4 py-3 rounded-xl shadow-sm">
        ...
        <div class="space-y-3.5">
            <div
                ...
                :class="[
                    isActive(field.key) ? 'border-indigo-300 dark:border-indigo-500/50 shadow-sm shadow-indigo-500/5' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600',
                    expandedFields[field.key] ? 'ring-1 ring-slate-900/5 dark:ring-white/5 shadow-md' : ''
                ]"
            >
                ...
                >
                    <div class="flex items-center gap-3.5">
                        ...
                        <div class="text-left">
                            <p class="font-bold text-slate-800 dark:text-slate-200 text-sm tracking-wide">{{ field.label }}</p>
                            <p class="text-xs font-medium text-slate-500 dark:text-slate-400 mt-0.5">{{ field.description }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        ...
                        <span 
                            ...
                            :class="localTokens[field.key]?.mode === 'color' 
                ...
                >
                    <div v-show="expandedFields[field.key]" class="border-t border-slate-100 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/30">
                        <div class="p-4 sm:p-5 space-y-6">
                            
                            <!-- Mode Selector (For Backgrounds) -->
                            <div v-if="!isTextColorOrShadowField(field.key)" class="space-y-2.5">
                                ...
                                <div class="grid grid-cols-3 gap-3">
                                    ...
                                    >
                                        <LuCircle class="w-5 h-5" :class="!localTokens[field.key]?.mode ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'" />
                                        <span class="text-xs font-bold" :class="!localTokens[field.key]?.mode ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-300'">Default</span>
                                    </button>
                                    ...
                                    >
                                        <div class="w-5 h-5 rounded-full border border-slate-300 dark:border-slate-600 shadow-inner" :style="{ background: localTokens[field.key]?.value || '#1f2937' }" />
                                        <span class="text-xs font-bold" :class="localTokens[field.key]?.mode === 'color' ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-300'">Solid Color</span>
                                    </button>
                                    ...
                                    >
                                        <LuImage class="w-5 h-5" :class="localTokens[field.key]?.mode === 'image' ? 'text-indigo-600 dark:text-indigo-400' : 'text-slate-400'" />
                                        <span class="text-xs font-bold" :class="localTokens[field.key]?.mode === 'image' ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-300'">Image URL</span>
                                    </button>
                                </div>
                            </div>

                            <!-- Color & Hex Input -->
                            <div v-if="localTokens[field.key]?.mode === 'color' || isTextColorOrShadowField(field.key)" class="space-y-2.5">
                            ...
                            <!-- Image URL Input -->
                            <div v-if="localTokens[field.key]?.mode === 'image'" class="space-y-2.5">
                            ...
                            <!-- Text Shadow Input -->
                            <div v-if="field.key === 'form-text-shadow'" class="space-y-2.5">
                            ...
                            <div class="flex items-center justify-between pt-5 mt-2 border-t border-slate-200/60 dark:border-slate-700/60">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    >
                                        <div v-if="!localTokens[field.key]?.value" class="absolute inset-0 bg-slate-100 dark:bg-slate-800 opacity-50"></div>
                                        <span class="relative z-10" :class="!localTokens[field.key]?.value ? 'text-slate-400' : ''">Aa</span>
                                    </div>
                                </div>
                                ...
                                >
                                    <LuRotateCcw class="w-3.5 h-3.5" /> Reset
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex items-center justify-between pt-6 border-t border-slate-200 dark:border-slate-800">
            <div class="text-[0.65rem] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
                <span class="text-indigo-600 dark:text-indigo-400">{{ fields.filter(f => isActive(f.key)).length }}</span> of {{ fields.length }} customized
            </div>
            <button
                type="button"
            ...
            >
                <LuTrash2 class="w-4 h-4" /> Reset All Fields
            </button>
        </div>
    </div>
</template>
