...
<template>
    <div :class="[$attrs.class, 'w-fit', { 'flex flex-col gap-1': label || error || hint, 'inline-flex': !label && !error && !hint }]">
        <div class="flex items-center gap-2">
            <input
                ...
                :disabled="disabled"
                :class="[
                    'rounded leading-none border-slate-300 text-indigo-600 focus:ring-indigo-600 dark:border-slate-700 dark:bg-slate-900 cursor-pointer',
                    error ? 'border-red-500 dark:border-red-500 focus:ring-red-500' : ''
                ]"
                v-bind="filteredAttrs"
            >
            <label v-if="label" :for="id" class="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer select-none">
                {{ label }}
            </label>
        </div>
