        ...
        <template
            ...
            <template v-if="mode === 'mobile'">
                ...
                <div
                    ...
                    class="ms-4 mt-1 flex flex-col gap-1 ps-3">
                    <ResponsiveNavLink
            ...
            <template v-else-if="mode === 'sidebar'">
                <template v-if="service.children && visibleChildren(service).length">
                    ...
                    <NavLink
                        v-if="service.href"
                        :href="route(service.href)"
                        :active="route().current(service.href)">
                        {{ service.label }}
                    </NavLink>
                    <NavLink
                        v-for="child in visibleChildren(service)"
            ...
            <template v-else>
                ...
                <Dropdown
                    v-else-if="service.children && visibleChildren(service).length"
                    align="right"
                    width="60">
                    <template #trigger>
                        <button class="inline-flex items-center rounded-md px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-200">
                            {{ service.label }}
                        </button>
                    </template>
