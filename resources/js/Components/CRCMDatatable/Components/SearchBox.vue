<template>
    <div class="flex flex-col gap-0.5">
        ...
        <div class="flex flex-row items-center justify-between gap-0.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-300 dark:border-slate-700 focus-within:border-lime-500 focus-within:ring-1 focus-within:ring-lime-500 transition-all overflow-hidden">
            <input
                autocomplete="off"
                class="border-0 py-1.5 px-3 w-full text-xs sm:text-sm bg-transparent text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:ring-0"
                type="text"
                id="dtSearch"
                v-model="search"
            ...
            <button
                v-if="modelValue"
                class="p-1.5 rounded-r hover:bg-slate-200 dark:hover:bg-slate-700 active:scale-90 transition-all text-slate-500 dark:text-slate-400"
                @click="clearSearch">
