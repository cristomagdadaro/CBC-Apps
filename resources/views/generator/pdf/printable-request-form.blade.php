                ...
                <td style="width:48%;padding:3mm;box-sizing:border-box;border:none;border-collapse:collapse;">
                    <div style="position:relative">
                        ...
                        <img src="{{ $logos['overlay'] }}"
                            style="height:90px;position:absolute;right:0;top:0;z-index:-1">
                    </div>
                    ...
                    <table style="border: none; border-collapse: collapse;">
                        ...
                        <tr>
                            ...
                            <td colspan="3">{{ $rf->request_purpose }}</td>
                        </tr>
                        <tr {{ empty($rf->request_details) ? 'style="display:none"' : '' }}>
                            <td colspan="1"><b>Details</b></td>
                            <td colspan="3">{{ $rf->request_details }}</td>
                        </tr>
                        <tr {{ empty($rf->project_title) ? 'style="display:none"' : '' }}>
                            <td colspan="1"><b>Project</b></td>
                            <td colspan="3">{{ $rf->project_title }}</td>
                        </tr>
                        <tr>
                            <td colspan="1">
                                <b>Date/Time</b>
                            </td>
                            <td colspan="1">{{ $rf->date_of_use }} {{ $rf->time_of_use }}</td>
                            <td colspan="1">
                                <b>Expected Return</b>
                            </td>
                            <td colspan="1">{{ $rf->date_of_use_end }} {{ $rf->time_of_use_end }}</td>
                        </tr>

                        @if (!empty($labs))
                            <tr>
                                <td colspan="1"><b>Laboratory</b></td>
                        ...
                        @if (!empty($equipments))
                            <tr>
                                ...
                                <td colspan="3">{{ $equipments }}</td>
                            </tr>
                        @endif

                        @if (!empty($supplies))
                            <tr>
                                <td colspan="1"><b>Supplies</b></td>
                    ...
                    <ul style="text-align: justify">
                        <li>I hereby acknowledge that I will utilize the supply/equipment/laboratory at my own risk; and
                            agree to use it
                            responsibly and in accordance with any provided instructions or safety guidelines
                        </li>
                        <li>I agree to assume full responsibility for any damage or loss of the equipment while it is in
                            my possession.</li>
                        <li>I agree that the Center shall not be held liable for the quality, accuracy, reliability, or
                            completeness of any
                            data generated by the Requestor using the lab's facilities, equipment, or resources. The
                            Requestor assumes full
                            responsibility for the design, execution, and interpretation of the experiments and the data
                            derived therefrom.
                            The Center makes no warranties, express or implied, regarding the outcomes of the
                            Requestor's research
                            activities.
                        </li>
                    </ul>
                    ...
                    <br>
                    @if ($form->approval_constraint)
                        <p><b>Approval Remarks:</b> {{ $form->approval_constraint }}</p>
                    @endif
                    @if ($form->disapproved_remarks)
                        <p><b>Disapproval Remarks:</b> {{ $form->disapproved_remarks }}</p>
                    @endif
